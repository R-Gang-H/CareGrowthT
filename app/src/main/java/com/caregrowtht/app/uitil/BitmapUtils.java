...
public class BitmapUtils {
    ...
    public static int decode(String code) {
        ...
        // 查找字符串最后一次出现的位置
        int lastIndex = code.lastIndexOf("0");
        if (lastIndex != -1) {
            System.out.println("0 字符串最后出现的位置： " + lastIndex);
            code = code.substring(lastIndex + 1);
        }

        int len = code.length();
        code = new StringBuilder(code).reverse().toString();// 倒序
        int num = 0;
        for (int i = 0; i < len; i++) {
            num += source_string.indexOf(code.toCharArray()[i]) * Math.pow(35, i);
        }
