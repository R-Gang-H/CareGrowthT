...
public class OrdersAdapter extends BaseQuickAdapter<String, BaseViewHolder> {
    ...
    public OrdersAdapter(Activity mActivity, int layoutResId, ArrayList<String> data, String today, int cardType) {
    ...
    protected void convert(BaseViewHolder helper, String item) {
        bubbleSort(listData);
        String dataTime = TimeUtils.dateTiem(selectDay, helper.getPosition(), "yyyy-MM-dd");
        helper.setText(R.id.tv_week, TextUtils.equals(today, dataTime) ? "今天" : Constant.weekArr[TimeUtils.getDayOfWeek(dataTime) - 1]);
        helper.setText(R.id.tv_date, TimeUtils.getDayOfWeek("dd", TimeUtils.getDayOfWeek(dataTime), dataTime));
        ExpandListView listView = helper.getView(R.id.lv_course);
        listView.setAdapter(new CourseItemAdapter(mActivity, getEveryDayData(TimeUtils.getDayOfWeek(dataTime) - 1), cardType, type));
        helper.setBackgroundColor(R.id.ll_header, TextUtils.equals(today, dataTime)
