...
public abstract class NineGridLayout extends ViewGroup {
    ...
    public String[] ShowImageOrVideo(String argUrls) {
        if (TextUtils.isEmpty(argUrls)) {
            return arrUrls;
        }
        arrUrls = argUrls.split("#");
        Log.e("-----------", "mDatas=" + argUrls);
        // String videoReg="(?i).+?\\.(mp4|gif|bmp|png)";
        return arrUrls;
    }
    //-------------------------------
    ...
    static public boolean CheckIsImage(String strUrl) {
...
//            pUrl = url.replace("mp4", "jpg");
        ...
        imageView.setScaleType(ImageView.ScaleType.FIT_XY);//fitXY的目标是填充整个ImageView，对图片进行一些缩放操作，在缩放的过程中，它不会按照原图的比例来缩放
        if (url.contains("mp4")) {
            imageView.setCenterImgShow(true);
        }
        imageView.setOnClickListener(v -> onClickImage(i, finalPUrl, mUrlList));
        return imageView;
    }
